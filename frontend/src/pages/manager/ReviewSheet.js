...
export default function ReviewSheet() {
  ...
  if (!sheet) return <div className="min-h-screen bg-gray-50"><Navbar /><div className="p-8 text-gray-400">Loading…</div></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <button onClick={() => navigate('/manager')} className="text-sm text-blue-600 hover:underline mb-4 block">← Back to Dashboard</button>
        <div className="flex items-center justify-between mb-6">
          ...
          <span className={`text-sm px-3 py-1 rounded-full font-medium capitalize ${sheet.status === 'submitted' ? 'bg-yellow-100 text-yellow-700' : sheet.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
            {sheet.status}
          </span>
        </div>
                  ...
                  <div className="flex-1">
                    ...
                    {isSubmitted && (
                      <div className="flex gap-3 mt-2">
                        ...
                        {edits[goal.id] && (
                          <button onClick={() => saveEdit(goal.id)} className="self-end text-xs bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700">
                            Save
                          </button>
                        )}
                      </div>
                    ...
                    {!isSubmitted && (
                      <span className="text-xs text-gray-400">Weight: {goal.weightage}% {goal.target_value ? `· Target: ${goal.target_value}` : ''}</span>
                    )}
                  </div>
                </div>
              </div>
        ...
        {sheet.status === 'submitted' && (
          <div className="flex gap-3">
            <button onClick={approve} className="bg-green-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-green-700">
              ✓ Approve & Lock
            </button>
            <button onClick={() => setShowReturn(true)} className="border border-red-300 text-red-600 px-5 py-2 rounded-lg text-sm font-medium hover:bg-red-50">
              Return for Rework
            </button>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
              ...
              <div className="flex gap-2">
                <button onClick={returnSheet} className="flex-1 bg-red-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-red-700">Send Back</button>
                <button onClick={() => setShowReturn(false)} className="flex-1 border border-gray-300 text-gray-700 py-2 rounded-lg text-sm hover:bg-gray-50">Cancel</button>
              </div>
            </div>
          </div>
        )}
